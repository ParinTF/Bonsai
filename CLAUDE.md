...
**One manual override on top of that: `ProgressCalculator.Effective(status, computed)`.** A goal with `Status == "done"` always reads 100%, applied *after* the per-type switch in `ComputeTreeAsync`, regardless of type — including a rollup whose children aren't all finished. This is the "Success" button on a rollup goal's editor: it forces that goal's own number to 100 without touching its children's status/progress at all, and since parents read children's already-written `Progress`, the override composes correctly up the tree. Un-marking done reverts to the real computed value on the next recompute — nothing is persisted specially, so there's no separate "override" flag to keep in sync.

**A "done" goal also hides its daily/weekly descendants from the surfaces that ask you to act on them.** `ProgressCalculator.HasDoneAncestor(ancestorIds, statusById)` is the shared pure check; `/today`, `/checkins` (the heatmap's `habitCount` denominator), and `/goals/this-week` all build a `goalId → status` map and filter it in, and the dashboard's client-side "To Do" section does the equivalent in `DashboardPage.tsx`. Nothing about the hidden goals themselves changes — they're not deleted, archived, or marked done; `GET /goals` (and the graph view) still shows them at their real status/progress. This is purely "don't nag me about a branch I've already called finished," so it's a filter at read time, not a cascading write.
...
- **SEO is scoped to `index.html` + `LandingPage.tsx` only, on purpose.** This is a client-rendered SPA behind a login wall — `/dashboard`, `/goals/:id`, `/today`, `/week`, `/review`, `/settings` all redirect an unauthenticated request to `/login` before any real content renders (`RequireAuth` in `App.tsx`), so there is nothing for a crawler to index there even without `robots.txt`'s `Disallow` lines backing it up. The three placeholder production URLs (`https://bonsai.pages.dev/` in `index.html`'s canonical/OG/Twitter tags, `public/robots.txt`'s `Sitemap:` line, and every `<loc>` in `public/sitemap.xml`) all need updating together once a real domain exists (see DEPLOY.md) — grep for `bonsai.pages.dev` to find every spot. `public/og-image.png` (1200×630, brand-colored card with a stylized goal-tree illustration, no screenshot text) already exists — regenerate by editing the HTML mockup and re-screenshotting at 2x with Playwright rather than hand-editing the PNG.
- Authenticated pages (`DashboardPage`, `GoalDetailPage`, `TodayPage`, `WeekPage`, `WeeklyReviewPage`, `SettingsPage`) are `React.lazy()`-loaded in `App.tsx` behind a `<Suspense>` per route — this keeps React Flow/Dagre (pulled in only by `GoalDetailPage`) out of the landing page's initial JS payload. Adding a new authenticated page should follow the same lazy pattern rather than a static import at the top of `App.tsx`.
