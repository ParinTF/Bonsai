...
**AI breakdown (BYOK).** `ILlmProvider` has one implementation per vendor (Anthropic/OpenAI/Gemini), each using its native structured-output mode; the schema is a *flat* list `{ tempId, parentTempId, title, progressType }` because no provider allows recursive schemas. User keys are validated live, encrypted with ASP.NET Data Protection (key ring persisted in Mongo via `MongoXmlRepository`), and only the last 4 chars are ever returned.
