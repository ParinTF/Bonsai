...
103 xUnit tests — pure-logic tests plus 4 `[SkippableFact]` integration tests that boot the real app against Mongo (and skip, not fail, when none is reachable):
...
- AI flat-list → tree conversion in [`BreakdownTreeBuilderTests.cs`](api/Bonsai.Api.Tests/BreakdownTreeBuilderTests.cs) — 2-level and 6-level builds with parents-first ordering and correct ancestor chains; stage/numeric payloads materialised only on their own type; responses with or without optional fields both parse; rejection (typed exception, no crash) of cycles, unknown/self/duplicate parent references, multiple roots, and >6-level depth
